...
const Hurricanes = () => {
  ...
  const handleSortCriteriaChange = (event: SelectChangeEvent) => {
    setSortCriteria(event.target.value as string);
  };

  const handleFilterCriteriaChange = (event: SelectChangeEvent) => {
  ...
  const getHurricanes = useCallback(async () => {
      ...
      >
        ...
        <div style={{ padding: "50px" }}>
          <Typography variant="subtitle1">
            Below is a list of hurricanes that affected Texas along with a few
            attributes of each hurricane including attributes such as their
            name, category, date, wind speed, and fatalities.{" "}
            <b>Number of hurricanes: </b>
            {numHurricanes}
          </Typography>
        </div>
